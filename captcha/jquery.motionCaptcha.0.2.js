...
jQuery.fn.motionCaptcha || (function($) {
	...
	$.fn.motionCaptcha = function(options) {
		...
		return this.each(function() {
			...
			//Check if a shape is already present
			for(var check=0; check<opts.shapes.length; check++)
			{
				if($canvas.hasClass(opts.shapes[check]))
				{
					$canvas.removeClass(opts.shapes[check]);
					opts.shapes.splice(check, 1);
				}
			}
	...
	$.fn.motionCaptcha.defaults = {
		...
		onSuccess: function($form, $canvas, ctx) {
			...
			document.getElementById('change-btn').setAttribute('disabled', '');
